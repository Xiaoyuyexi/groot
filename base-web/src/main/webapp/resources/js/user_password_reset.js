...
function merchantPasswordReset() {
	var opratorId=$("#opratorId").val();
		$.ajax({
			  url: contextPath +"/updatePassword.jhtml?r="+Math.random(),
			  async:false,
			  type:"POST",
			  data:{"opratorId":opratorId},
			  success: function(result) {
				  showMessageOfParent(result.content);
...
function merchantSearch() {
	...
	var opratorNo=$("#opratorNo").val();
	if (merchantPhone == '' || opratorNo == '') {
		showMessageOfParent("请输入商户手机号和操作员编号!");
		setTimeout("hideMessageOfParent()", "3000");
	}else{
		$.ajax({
			  url: contextPath +"/getMerchantByPhone.jhtml?r="+Math.random(),
			  async:false,
			  type:"POST",
			  ...
			  success: function(result) {
				  	if (result.isSuccess == true) {
				  		$("#merchantNameSpan").text(result.obj.merchantName);
				  		$("#merchantPhoneSpan").text(result.obj.merchantPhone);
				  		$("#merchantEmailSpan").text(result.obj.merchantEmail);
				  		$("#opratorNameSpan").text(result.obj.opratorName);
				  		$("#opratorId").val(result.obj.opratorId);
				  		$("#merchantInfo").show();
				  	}else{
				  		showMessageOfParent(result.content);
				  		setTimeout("hideMessageOfParent()", "3000");
